# experiments/exp_B3_fish_no_rest.py
# B3: Fish is not allowed to target/commit to Rest (Rest still exists for other agents)
...
# --- ensure project root is on sys.path so "import profiles" works when running from /experiments ---
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

from profiles import get_profile, MotionProfile  # noqa: E402
...
def ensure_telemetry_paths() -> str:
    ...
    # tag the run so you can see which experiment made it
    return os.path.join(out_dir, f"run_{ts}_B3_fish_no_rest_ecosystem.csv")
...
# ✅ FIXED: pad auto-shrinks for small zones (Transition/Rest safe)
...
def pick_commit_zone_fish(agent: Agent, zones: List[Zone]) -> Optional[str]:
    """Fish occasional 'commit' choice. B3: never commit to Rest."""
    p = agent.profile
...
def decide_target_fish(agent: Agent, zones: List[Zone]) -> pygame.Vector2:
    """Fish smooth 'sprint + rest' style target. B3: never target Rest."""
    p = agent.profile
    s = agent.state

    # if committed, go to that zone center
    if agent.fish_commit_ticks_left > 0 and agent.fish_commit_zone is not None:
        if agent.fish_commit_zone == "Rest":
            agent.fish_commit_zone = None  # safety
        else:
            for z in zones:
    ...
    if current is not None and current.name != "Rest" and agent.dwell_ticks < p.fish_min_dwell_ticks:
...
def update_agent_fish(agent: Agent, zones: List[Zone], dt: float):
    ...
    # ✅ B3: fish ignores Rest zone effects (treat as None)
    if z is not None and z.name == "Rest":
        apply_zone_effects(agent, None, dt)
    else:
        apply_zone_effects(agent, z, dt)

    # edge pauses still allowed anywhere (including Rest edge) – if you want NO Rest interaction at all,
    # we can also skip maybe_edge_pause_fish when z.name == "Rest".
...
# -------------------------
# BIRD model (reuse ANT engine)
# -------------------------
def update_agent_bird(agent: Agent, zones: List[Zone], dt: float):
...
def draw_hud(screen: pygame.Surface, font: pygame.font.Font, hud_on: bool, agents: List[Agent], csv_path: str):
    ...
    lines = [
        ...
        "Agents: A=FISH  B=ANT_FAST  C=ANT_SLOW  D=BIRD",
        "EXP: B3 (fish_no_rest)",
    ]
    ...
    for a in agents:
        s = a.state
        line = (
            f"{a.agent_id}  {a.profile.name:<12}  Z:{a.current_zone:<10}  "
            f"D:{a.dwell_ticks:>3}  paused:{'1' if a.paused else '0'}  "
...
def main():
    ...
    pygame.display.set_caption("ORPIN / MOS Sandbox Town — B3 (Fish no Rest)")
    ...
    bird_profile = get_profile("bird")  # must exist in profiles package
    ...
    bird = Agent(agent_id="D", profile=bird_profile, pos=v2(500, 240), vel=v2(0, 0), target=zmap["Transition"].center())

    # init ant commits + trails
        ...
        while running:
            ...
            if ran:
                ...
                for agent in agents:
                    ...
                    elif agent.profile.model == "bird":
                        update_agent_bird(agent, zones, dt)
