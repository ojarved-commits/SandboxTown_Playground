...
def ensure_telemetry_paths() -> str:
    ...
    return os.path.join(out_dir, f"run_{ts}_two_agents.csv")
...
def draw_world(screen: pygame.Surface, zones: List[Zone], agents: List[Agent]):
    ...
    for a in agents:
        p = a.profile
        col = AGENT_A_COLOR if a.agent_id == "A" else AGENT_B_COLOR
...
def draw_hud(screen: pygame.Surface, font: pygame.font.Font, hud_on: bool, agents: List[Agent], csv_path: str):
    ...
    lines = [
        ...
        f"Agents: A=FISH  B=ANT  | telemetry: {csv_path}",
    ]
...
def main():
    ...
    ant_profile = get_profile("ant_slow")

    a = Agent(agent_id="A", profile=fish_profile, pos=v2(250, 310), vel=v2(0, 0), target=zmap["Library"].center())
    b = Agent(agent_id="B", profile=ant_profile, pos=v2(720, 310), vel=v2(0, 0), target=zmap["Park"].center())

    # init ant commit data
    b.commit_zone = zmap["Library"]
    b.commit_ticks = random.randint(ant_profile.commit_min, ant_profile.commit_max)
    b.last_choice = "Library"
    b.target = pick_point_in_zone(b.commit_zone, pad=55)
    ...
    try:
        while running:
            ...
            # clamp by the stricter of the two profiles
            dt = min(dt, min(a.profile.dt_clamp, b.profile.dt_clamp))
