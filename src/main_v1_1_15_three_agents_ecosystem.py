...
# agent colors (keep simple + distinct)
COLOR_FISH = (210, 255, 230)
COLOR_ANT_FAST = (255, 230, 210)
COLOR_ANT_SLOW = (200, 240, 255)
...
def ensure_telemetry_path(profiles: List[MotionProfile]) -> str:
    out_dir = os.path.join("telemetry", "runs")
    os.makedirs(out_dir, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    tag = "_".join([p.name.lower() for p in profiles])  # fish_aquarium_ant_fast_ant_slow
    return os.path.join(out_dir, f"run_{ts}_three_agents_{tag}.csv")
...
# Draw
# -------------------------
def agent_color(a: Agent) -> Tuple[int, int, int]:
    if a.profile.model == "fish":
        return COLOR_FISH
    if a.profile.name == "ANT_FAST":
        return COLOR_ANT_FAST
    return COLOR_ANT_SLOW
...
def draw_hud(screen: pygame.Surface, font: pygame.font.Font, hud_on: bool, agents: List[Agent], csv_path: str):
    ...
    for a in agents:
        s = a.state
        if a.profile.model == "fish":
            commit_zone = a.fish_commit_zone if a.fish_commit_zone else "-"
            commit_left = a.fish_commit_ticks_left
        else:
            commit_zone = a.commit_zone.name if a.commit_zone else "-"
            commit_left = a.commit_ticks

        line = (
            f"{a.agent_id}  {a.profile.name:<12}  "
            f"Z:{a.current_zone:<10}  D:{a.dwell_ticks:>3}  paused:{'1' if a.paused else '0'}  "
            f"commit:{commit_zone}({commit_left:>3})  "
...
def init_ant_commit(agent: Agent, zmap: Dict[str, Zone]):
    # mirrors your known-good ant init
    agent.commit_zone = zmap["Library"]
    agent.commit_ticks = random.randint(agent.profile.commit_min, agent.profile.commit_max)
    agent.last_choice = "Library"
    agent.target = pick_point_in_zone(agent.commit_zone, pad=55)
    agent.trail = [agent.pos.copy()]
...
def main():
    ...
    # spawn positions: near library, but spaced
    fish = Agent(agent_id="A", profile=fish_profile, pos=v2(250, 310), vel=v2(0, 0), target=zmap["Library"].center())
    ant_fast = Agent(agent_id="B", profile=ant_fast_profile, pos=v2(280, 340), vel=v2(0, 0), target=zmap["Library"].center())
    ant_slow = Agent(agent_id="C", profile=ant_slow_profile, pos=v2(220, 270), vel=v2(0, 0), target=zmap["Library"].center())

    init_ant_commit(ant_fast, zmap)
    init_ant_commit(ant_slow, zmap)

    agents = [fish, ant_fast, ant_slow]
    ...
    csv_path = ensure_telemetry_path([fish_profile, ant_fast_profile, ant_slow_profile])
